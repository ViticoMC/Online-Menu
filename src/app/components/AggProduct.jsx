...
export default function AgregarProducto() {
  ...
  const handleImageChange = async(e) => {
    ...
    try{
       ...
       await setImageFile(result.secure_url)
    }catch(e){
      console.log("error", e)
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    .from("products")
    .insert({  name: formData.name, price: formData.price, material: formData.material  , image : imageFile});

    console.log(error)

    setIsAggProduct(false);
  ...
  return (
    ...
    <Card className="w-full max-w-md bg-[#120824] border-[#2a1a3a] text-white fixed z-1000 top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]">
      <CardHeader className="border-b border-[#2a1a3a] flex justify-between items-center">
        <CardTitle className="text-[#ff00ff]">Añadir Nuevo Piercing</CardTitle>
